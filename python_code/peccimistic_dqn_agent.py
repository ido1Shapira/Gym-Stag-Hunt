...
class HumamModel:
  def __init__(self, agent_model):
    self.model = tf.keras.models.load_model('./data/humanModel/dropout_model_v0.h5')
  ...
  def predict_action(self, state):
    ...
    Q = self.agent_model.predict(state)[0]
...
def run(env, episodes = 1100, epsilon_decay=0.9975, train=False, beta = 0.5, SARL=False):
  ...
  if not train:
    ...
    if SARL:
      agent_model.load("data/weights/peccimistic_SARL_ddqn_agent"+"_"+str(beta)+"_"+str(episodes)+"_"+str(epsilon_decay)+".h5")
    else:
      agent_model.load("data/weights/peccimistic_ddqn_agent"+"_"+str(episodes)+"_"+str(epsilon_decay)+".h5")
    episodes = 5
  else:
    agent_model = DQNAgent((5,5,3), env.action_space.n, epsilon_decay, 1.0)
  
  human_model = HumamModel(agent_model.model)
  monitor = Monitor()
  ...
  for ep in range(episodes):    
    ...
    if train:
      ...
      if SARL:
        average = monitor.PlotModel(ep_reward, ep_human_reward, ep, "peccimistic_SARL_ddqn_agent"+"_"+str(beta)+"_"+str(episodes)+"_"+str(epsilon_decay))
      else:
        average = monitor.PlotModel(ep_reward, ep_human_reward, ep, "peccimistic_ddqn_agent"+"_"+str(episodes)+"_"+str(epsilon_decay))
  ...
  if train:
    if SARL:
      agent_model.save("data/weights/peccimistic_SARL_ddqn_agent"+"_"+str(beta)+"_"+str(episodes)+"_"+str(epsilon_decay)+".h5")
    else:
      agent_model.save("data/weights/peccimistic_ddqn_agent"+"_"+str(episodes)+"_"+str(epsilon_decay)+".h5")
  env.close()
